...
// 1. Detectar el inicio del toque o clic
function startDrag(e) {
    // Evitar que el giro se active si tocamos el panel de control
    if (e.target.closest('#dashboard') || e.target.closest('#floatingButtons') || e.target.closest('#infoModal')) {
        return;
    }
...
// 2. Detener el giro al soltar
function stopDrag() {
    isDragging = false;
}

// 3. Calcular y aplicar la rotación mientras se arrastra
function drag(e) {
    ...
    // Obtener la rotación actual del contenedor general
    const currentRotation = centrifugeContainer.getAttribute("rotation");
    
    // Aplicar la nueva rotación (multiplicamos por 0.5 para que no gire tan rápido)
...
// Asignar los eventos para Mouse (Computador)
document.addEventListener("mousedown", startDrag);
document.addEventListener("mouseup", stopDrag);
document.addEventListener("mousemove", drag);

// Asignar los eventos para Pantalla Táctil (Celular/Tablet)
